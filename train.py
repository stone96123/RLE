...
from random_erasing import RandomErasing, ModerateRLE, RadicalRLE
...
suffix = dataset
if args.method=='agw':
    suffix = suffix + '_RLE_p{}_n{}_lr_{}_seed_{}'.format(args.num_pos, args.batch_size, args.lr, args.seed)
else:
...
transform_sysu = transforms.Compose([
    ...
    RandomErasing(probability = args.erasing_p),
    normalize,
])
...
if args.optim == 'sgd':
    ...
    optimizer = optim.SGD([
        ...
        {'params': net.bottleneckg.parameters(), 'lr': args.lr},
        ...
        {'params': net.classifierg.parameters(), 'lr': args.lr}],
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        feat1, feat2, feat3, featg, out1, out2, out3, outg = net(input1, input2)

        loss_id_p = (criterion_id(out1, labels) + criterion_id(out2, labels) + criterion_id(out3, labels))/3
        loss_id_g = criterion_id(outg, labels)
        loss_id = loss_id_p + loss_id_g
        
        loss_tri_p = (criterion_tri(feat1, labels) + criterion_tri(feat2, labels) + criterion_tri(feat3, labels))/3
        loss_tri_g = criterion_tri(featg, labels)
        loss_tri = loss_tri_p + loss_tri_g
...
def test(epoch):
    ...
    ptr = 0
    if dataset == 'regdb':
        gall_feat1 = np.zeros((ngall, 1024 * 4))
        gall_feat2 = np.zeros((ngall, 1024 * 4))
    elif dataset == 'sysu':
        gall_feat1 = np.zeros((ngall, 2048 * 4))
        gall_feat2 = np.zeros((ngall, 2048 * 4))
    elif dataset == 'llcm':
        gall_feat1 = np.zeros((ngall, 2048 * 4))
        gall_feat2 = np.zeros((ngall, 2048 * 4))
    with torch.no_grad():
    ...
    ptr = 0
    if dataset == 'regdb':
        query_feat1 = np.zeros((nquery, 1024 * 4))
        query_feat2 = np.zeros((nquery, 1024 * 4))
    elif dataset == 'sysu':
        query_feat1 = np.zeros((nquery, 2048 * 4))
        query_feat2 = np.zeros((nquery, 2048 * 4))
    elif dataset == 'llcm':
        query_feat1 = np.zeros((nquery, 2048 * 4))
        query_feat2 = np.zeros((nquery, 2048 * 4))
    with torch.no_grad():
